...
bool ls_dxf_process_entity(lsDxf* dxf)
{
	...
	while (1)
	{
		...
		if (0 == code && ls_utils_is_string_equal_n(rowString, "ENDSEC", 6))
			return true;

		// 处理线段
		if (ls_utils_is_string_equal_n(rowString, "LINE", 4))
		{
...
bool ls_dxf_process_line(lsDxf* dxf)
{
	...
	while (1)
	{
		...
		case 0:
		{
			...
			ls_log_info("Segment start, x : %.3f, y : %.3f\n", segment.s.x, segment.s.y);
			ls_log_info("Segment end, x : %.3f, y : %.3f\n", segment.e.x, segment.e.y);
			// 处理线段
			if (ls_utils_is_string_equal_n(dxf->str, "LINE", 4))
			{
				ls_dxf_process_line(dxf);
			}
			//ls_shapes_add_line(shapes, segment);
			return true;
		}
		break;
		...
		case 30:
		{
			segment.s.z = (lsReal)atof(ls_dxf_get_row_string(dxf));
		}
		break;
		...
		case 31:
		{
			segment.e.z = (lsReal)atof(ls_dxf_get_row_string(dxf));
		}
		break;
		}
	}
